...
const DeployProject = ({ expandRightPanel, setShowAiModal }: Props) => {
  ...
  useEffect(() => {
    async function getWorkspaces() {
      ...
      try {
        ...
        toast(<ErrorToast title="Error !!!" description={"Could not load yam. Please try again later."} />)
        setLoading(false);
      }
    }
    getWorkspaces();
  ...
  const handleDeployWordPress = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast(<SuccessToast title="Success !!!" description="WordPress deployment created successfully!" />)
        ...
        toast(<ErrorToast title="Error !!!" description={result.error} />)
      }
    } catch (error) {
      setShowAnimation(false);
      console.error("Failed to deploy WordPress:", error);
      toast(<ErrorToast title="Error !!!" description={"Failed to deploy WordPress. Please try again."} />)
    }
  };
  ...
  const handleDeployCodeServer = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast(<SuccessToast title="Success !!!" description="CodeServer deployment created successfully!" />)
        setShowAnimation(false);
        setTimeout(() => {
          router.back();
        }, 5000);
      ...
      toast(<ErrorToast title="Error !!!" description="Failed to deploy CodeServer. Please try again." />)
    }
  };
  ...
  const handleDeployN8n = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast(<SuccessToast title="Success !!!" description="n8n deployment created successfully!" />)
        setShowAnimation(false);
        setTimeout(() => {
          router.back();
        }, 5000);
      ...
      toast(<ErrorToast title="Error !!!" description="Failed to deploy n8n. Please try again." />)
    }
  };
